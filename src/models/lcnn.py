...
class FrontendLCNN_lfccmfcc(LCNN):
    """ Model definition
    """
    def __init__(self, device: str = "cuda", **kwargs):
        super().__init__(**kwargs)
...
class LCNNContext(torch_nn.Module):
    """ Model definition
    """

    def __init__(self, context="no", **kwargs):
        super().__init__()
        ...
        ### modified
        self.context_fc_dim = 128

        self.dropout = torch_nn.Dropout(0.5)

        self.encoder_c = torch_nn.Sequential(
            torch_nn.Linear(self.embedding_dim, 128),  # 3081 1540 777
            torch_nn.LeakyReLU(0.1),
            ...
            # # nn.ReLU(),
            # nn.LeakyReLU(0.1),
            torch_nn.Linear(128, self.context_fc_dim),
            torch_nn.LeakyReLU(0.1),
        )

        self.fc2_o_c = torch_nn.Sequential(
            torch_nn.Linear(768 + self.context_fc_dim, 768),  # 3081 1540 777
            torch_nn.LeakyReLU(0.1),
            ...
            # # nn.ReLU(),
            # nn.LeakyReLU(0.1),
            torch_nn.Linear(768, self.v_emd_dim),
            # nn.LeakyReLU(0.1),
        )
    def _compute_embedding(self, x, c_e):
        ...
        if self.context == "no":
            ...
            #  5. pass through the output layer
            x_a = (hidden_features_lstm + hidden_features).mean(1)
            x_c = self.encoder_c(c_e)
            x = torch.cat((x_a, x_c), 1)

            x = self.dropout(x)
            tmp_emb = self.fc2_o_c(x)
...
class FrontendLCNNContext(LCNNContext):
    """ Model definition
    """
    def __init__(self, device: str = "cuda", **kwargs):
        super().__init__(**kwargs)
    ...
    def forward(self, x, **kwargs):
        x = self._compute_frontend(x)
        feature_vec = self._compute_embedding(x, **kwargs)

        return feature_vec

class FrontendLCNNContext_lfccmfcc(LCNNContext):
    """ Model definition
    """
    def __init__(self, device: str = "cuda", **kwargs):
        super().__init__(**kwargs)
