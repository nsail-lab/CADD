...
class RawNet3_context(nn.Module):
    def __init__(self, block, model_scale, context, summed, embedding_dim, context_jdd, C=1024, **kwargs):
        super().__init__()
        self.context_jdd = context_jdd
        ...
        ### modified
        self.context_fc_dim = 384

        self.bn6 = nn.BatchNorm1d(3072 + self.context_fc_dim)
        self.encoder_c = nn.Sequential(
            nn.Linear(self.embedding_dim, 384),  # 3081 1540 777
            nn.LeakyReLU(0.1),
            nn.Linear(384, 384),
            nn.LeakyReLU(0.1),
            nn.Linear(384, 384),
            ...
            # # nn.ReLU(),
            # nn.LeakyReLU(0.1),
            nn.Linear(384, self.context_fc_dim),
            nn.LeakyReLU(0.1),
        )

        self.fc2_o_c = nn.Sequential(
            nn.Linear(3072 + self.context_fc_dim, 3072),  # 3081 1540 777
            nn.LeakyReLU(0.1),
            nn.Linear(3072, 3072),
            nn.LeakyReLU(0.1),
            # nn.Linear(512, 128),
...
def prepare_model_context(embedding_dim, context_jdd):
    model = RawNet3_context(
        Bottle2neck,
        model_scale=8,
        context=True,
        summed=True,
